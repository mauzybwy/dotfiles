;;; mauzy-splash.el --- Custom splash screen with image and text -*- no-byte-compile: t; lexical-binding: t; -*-
...
;;   (setq mauzy/splash-image-path "~/path/to/image.png")
;;   (setq mauzy/splash-text '("Welcome back!" "Have a great day :)"))
...
(defcustom mauzy/splash-image-path nil
...
(defcustom mauzy/splash-text '("Welcome to Emacs!")
...
(defun mauzy/splash--calculate-padding (content-width window-width)
  "Calculate padding to center CONTENT-WIDTH in WINDOW-WIDTH."
  (max 0 (/ (- window-width content-width) 2)))

(defun mauzy/splash--calculate-vertical-offset (image-height text-lines window-height)
...
(defun mauzy/splash--insert-centered-image (image window-width)
  "Insert IMAGE centered in WINDOW-WIDTH."
  (let* ((img-width (car (image-size image t)))
         (char-width (frame-char-width))
         (img-cols (/ img-width char-width))
         (padding (mauzy/splash--calculate-padding img-cols window-width)))
    (insert (make-string padding ?\s))
    (insert-image image)))

(defun mauzy/splash--insert-centered-text-lines (text-lines window-width)
  "Insert TEXT-LINES, each centered in WINDOW-WIDTH."
  (dolist (line text-lines)
    (let ((padding (mauzy/splash--calculate-padding (length line) window-width)))
      (insert (make-string padding ?\s))
      (insert line "\n"))))

(defun mauzy/splash--cleanup-buffer ()
...
(defun mauzy/splash--draw-content ()
  ...
  (let ((inhibit-read-only t))
    ...
    (if (and mauzy/splash-image-path (file-exists-p mauzy/splash-image-path))
        ;; Draw with image
        (let* ((img (create-image mauzy/splash-image-path))
               (img-size (image-size img t))
               (img-width (car img-size))
               (img-height (cdr img-size))
               (win-width (window-width))
               (win-height (window-height))
               (v-offset (mauzy/splash--calculate-vertical-offset 
                          img-height (length mauzy/splash-text) win-height)))
          
          ;; Vertical spacing
          (insert (make-string v-offset ?\n))
          
          ;; Insert image
          (mauzy/splash--insert-centered-image img win-width)
          (insert "\n\n")
          
          ;; Insert text
          (mauzy/splash--insert-centered-text-lines mauzy/splash-text win-width)
          
          ;; Cleanup
          (mauzy/splash--cleanup-buffer))
      
      ;; Draw text only (no image)
      (let* ((win-width (window-width))
             (win-height (window-height))
             (v-offset (max 0 (/ (- win-height (length mauzy/splash-text)) 2))))
...
(provide 'mauzy-splash)

;;; mauzy-splash.el ends here
